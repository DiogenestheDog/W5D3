...
def no_apples_for_blair_sub
  ...
  FROM cats
  WHERE cats.name != 'Blair' AND cats.name IN(
    SELECT cats.name
    FROM cats
    JOIN cattoys ON cats.id = cattoys.cat_id
    JOIN toys on toys.id = cattoys.toy_id
...
def price_like_shiny_mouse
  ...
  FROM toys 
  JOIN toys AS b ON toys.price = b.price
  WHERE toys.name = 'Shiny Mouse' AND b.name != 'Shiny Mouse'
  ORDER By b.name
...
def just_like_orange
  ...
  # DO NOT USE A SUBQUERY
  execute(<<-SQL)
  SELECT b.name, b.breed
  FROM cats
  JOIN cats AS b ON cats.breed = b.breed
  WHERE cats.name = 'Orange' AND b.name != 'Orange'
  ORDER BY b.name
  SQL
end
...
def just_like_orange_sub
  ...
  FROM cats
  WHERE cats.name != 'Orange' AND cats.breed IN (
    SELECT cats.breed
    FROM cats
...
def toys_that_jet_owns
  ...
  # DO NOT USE A SUBQUERY
  execute(<<-SQL)
  SELECT DISTINCT cats_B.name, toys_B.name
  FROM toys
  JOIN toys AS toys_B ON toys.id = toys_B.id
  JOIN cattoys AS cattoys_B ON toys_B.id = cattoys_B.toy_id
  JOIN cats AS cats_B ON cattoys_B.cat_id = cats_B.id
  JOIN cattoys ON cattoys.toy_id = toys.id
  JOIN cats ON cats.id = cattoys.cat_id
  WHERE cats.name = 'Jet' AND cats_B.name != 'Jet'
  ORDER BY cats_B.name
